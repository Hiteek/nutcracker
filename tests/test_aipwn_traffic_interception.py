...
co-piloto no podía ver tráfico de una app Flutter real (com.joinnus.
joinnus_organizer) porque `sniff_network_calls` solo hookea el stack Java.
...
def test_capture_traffic_parses_java_and_native_events(tmp_path):
    raw_output = "\n".join([
        '[CAP-JAVA-REQ] ' + json.dumps({"method": "POST", "url": "https://api.joinnus.com/login", "headers": "Auth: x", "body": '{"user":"a"}'}),
        '[CAP-JAVA-RESP] ' + json.dumps({"url": "https://api.joinnus.com/login", "code": 200, "headers": "", "body": '{"token":"t"}'}),
    ...
    assert java_req["url"] == "https://api.joinnus.com/login"
